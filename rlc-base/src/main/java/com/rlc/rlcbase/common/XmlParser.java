...
public class XmlParser {
    ...
    public Document str2doc(String docStr){
        try {
            ...
            Document document= (Document) builder.parse(bin);
            return document;
        } catch (Exception e) {
            e.printStackTrace();
            return null;
     ...
     * @return document
     */
    public Document strXML2Doc(String requestXML) {

        Document document = null;
        try {
            document = DocumentHelper.parseText(requestXML);
        } catch (DocumentException e) {
            log.error("解析数据错误，请检查文本" + e.getMessage());
        }
//        Element root = document.getRootElement();
//        Iterator<Element> ment = root.elementIterator();
//        while (ment.hasNext()) {
//            Element m = ment.next();
//            System.out.println(m.getName() + " / " + m.getTextTrim());
//        }
        return document;
    }
